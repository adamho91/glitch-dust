...
- New scenes default to **Typewriter + word rise**: characters type on in reading order, and each word rises to its fixed final position with out-quad easing over the entrance length (0.6 seconds by default). Prompt blocks follow when Reveal prompt words is enabled. Existing saved entrance choices remain intact; choose another option in Type → Entrance.
...
- Import images and video, place an asset in a scene, or use **Build scenes from all media** to generate a sequence. Scenes have one media slot, or two independent slots in comparison layouts, with fit/crop, zoom, focal position, darkening, clip start, and looping controls.
- Twelve new **Media** templates: Before / after, Stacked comparison, Old / new feature, Overview / detail, Two feature demos, Comparison strips, Feature launch, Feature notes, What changed, Feature spotlight, Release update, and Demo / takeaway. They follow the existing Focal headlines, prompt labels, square frames, and palette controls across all four formats. **What changed** uses the first three nonempty caption lines as feature notes.
- Comparison layouts support two different images or silent videos. In **Media → Place / edit media**, choose A or B, then click an upload; uploading two files together from A fills both slots. Edit labels and each slot’s crop, zoom, darkening, clip start and looping separately. Clicking a frame selects its slot for removal. Both sources persist through save/open and appear in SVG/still/MP4 exports. Comparison layouts use the chosen scene transition (dissolve when first selected); Media flow continues to apply to single-source layouts.
...
```sh
node --test scripts/test-video-core.mjs scripts/test-video-import.mjs scripts/test-video-media-layouts.mjs
```
